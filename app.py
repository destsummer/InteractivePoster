...
def create_poster():
    ...
    my_poster.add_section(title="Abstract",
        text="Suicide is a major national health concern. Over the last twenty years, the suicide rate of veterans has increased in the US by almost 50%. Adoption of Electronic Health Records (EHR) has led to a sharp increase in clinical data in forms of structured data, such as demographic information, medications, billing codes, etc, and unstructured data such as free format physician notes. These notes often include information that is not usually captured by structured data. Processing, formatting, and parsing of such notes are essential in helping produce clinical decisions. Latent Dirichlet Allocation (LDA) is a statistical model that allows us to find topics in such free format text. Preliminary work showed that our LDA models are sensitive enough to pick up different topics related to determinants of suicide between genders. Currently, we’ve been working with doctors at the U.S Department of Veterans Affairs (VA) and using a secondary natural language processing (NLP) tool called Clever, which extracts precise clinical concepts. Our results have shown that some patient information found within the free text is missing from the structured data. This missing data can lead to incomplete and incorrect analysis. Finding and supplementing this information will provide the VA doctors a better understanding of the patients at risk.",
